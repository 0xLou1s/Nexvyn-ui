export function highlightCode(code: string): string {
  ...
  const keywords = /\b(const|let|var|function|return|import|export|from|default|as|true|false|null|undefined|if|else|for|while|switch|case|break|class|extends|new|this|type|interface)\b/g
  html = html.replace(keywords, '<span class="token-keyword">$1</span>')

  html = html.replace(/(&lt;\/?[A-Z][a-zA-Z0-9_-]*|&lt;\/?[a-z][a-zA-Z0-9_-]*)/g, '<span class="token-tag">$1</span>')
  html = html.replace(/(\/?&gt;)/g, '<span class="token-tag">$1</span>')

  html = html.replace(/\b(className|style|id|onClick|type|variant|size)\b=/g, '<span class="token-attr">$1</span>=')
